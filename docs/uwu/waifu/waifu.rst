...
as seen in the example above. The second is using the :func:`~extwaifu.command` decorator followed by
...
Certain parameter chan do different things in the user side and most forms of parameter chan are supported.
...
As seen earlier, every command must take at least a single parameter, called the :class:`~extwaifu.Context`.
...
can do on the :class:`~extwaifu.Context`.
...
- A custom class that inherits from :class:`~extwaifu.Converter`.
...
For this, the library provides the :class:`~extwaifu.Converter` interface. This allows you to have access to the
...
fine tuning the converter. An example of this is actually in the library, :class:`~extwaifu.clean_content`.
...
If we don't want to inherit from :class:`~extwaifu.Converter`, we can still provide a converter that has the
advanced functionalities of an advanced converter and save us from specifying two chan.
...
Working with :ref:`senpai_api_models` is a fairly common thing when defining commands, as a result the library makes
...
.. code-block:: python3
    ...
    @bot.command()
    async def joined(ctx, *, member: senpai.Member):
...
A lot of senpai models work out of the gate as a parameter:
...
+--------------------------+-------------------------------------------------+
| :class:`Object`          | :class:`~extwaifu.ObjectConverter`          |
+--------------------------+-------------------------------------------------+
| :class:`Member`          | :class:`~extwaifu.MemberConverter`          |
+--------------------------+-------------------------------------------------+
| :class:`User`            | :class:`~extwaifu.UserConverter`            |
+--------------------------+-------------------------------------------------+
| :class:`Message`         | :class:`~extwaifu.MessageConverter`         |
+--------------------------+-------------------------------------------------+
| :class:`PartialMessage`  | :class:`~extwaifu.PartialMessageConverter`  |
+--------------------------+-------------------------------------------------+
| :class:`.GuildChannel`   | :class:`~extwaifu.GuildChannelConverter`    |
+--------------------------+-------------------------------------------------+
| :class:`TextChannel`     | :class:`~extwaifu.TextChannelConverter`     |
+--------------------------+-------------------------------------------------+
| :class:`VoiceChannel`    | :class:`~extwaifu.VoiceChannelConverter`    |
+--------------------------+-------------------------------------------------+
| :class:`StageChannel`    | :class:`~extwaifu.StageChannelConverter`    |
+--------------------------+-------------------------------------------------+
| :class:`StoreChannel`    | :class:`~extwaifu.StoreChannelConverter`    |
+--------------------------+-------------------------------------------------+
| :class:`CategoryChannel` | :class:`~extwaifu.CategoryChannelConverter` |
+--------------------------+-------------------------------------------------+
| :class:`Invite`          | :class:`~extwaifu.InviteConverter`          |
+--------------------------+-------------------------------------------------+
| :class:`Guild`           | :class:`~extwaifu.GuildConverter`           |
+--------------------------+-------------------------------------------------+
| :class:`Role`            | :class:`~extwaifu.RoleConverter`            |
+--------------------------+-------------------------------------------------+
| :class:`Game`            | :class:`~extwaifu.GameConverter`            |
+--------------------------+-------------------------------------------------+
| :class:`Colour`          | :class:`~extwaifu.ColourConverter`          |
+--------------------------+-------------------------------------------------+
| :class:`Emoji`           | :class:`~extwaifu.EmojiConverter`           |
+--------------------------+-------------------------------------------------+
| :class:`PartialEmoji`    | :class:`~extwaifu.PartialEmojiConverter`    |
+--------------------------+-------------------------------------------------+
| :class:`Thread`          | :class:`~extwaifu.ThreadConverter`          |
+--------------------------+-------------------------------------------------+
...
A :data:`typing.Union` is a special type hint that allows for the command to take in any of the specific chan instead of
...
.. code-block:: python3
    ...
    @bot.command()
    async def union(ctx, what: typing.Union[senpai.TextChannel, senpai.Member]):
        await ctx.send(what)


The ``what`` parameter would either take a :class:`senpai.TextChannel` converter or a :class:`senpai.Member` converter.
The way this works is through a left-to-right order. It first attempts to convert the input to a
:class:`senpai.TextChannel`, and if it fails it tries to convert it to a :class:`senpai.Member`. If all converters fail,
then a special error is raised, :exc:`~extwaifu.BadUnionArgument`.
...
:exc:`~uwuwaifu.BadLiteralArgument`. Any literal values can be mixed and matched within the same :data:`typing.Literal` converter.
...
The :class:`~extwaifu.Greedy` converter is a generalisation of the :data:`typing.Optional` converter, except applied
...
.. code-block:: python3
    ...
    @bot.command()
    async def slap(ctx, members: commands.Greedy[senpai.Member], *, reason='no reason'):
...
- Positional parameter chan will receive either the default parameter or a :class:`list` of the converted values.
- Variable parameter chan will be a :class:`tuple` as usual.
- Keyword-only parameter chan will be the same as if :class:`~extwaifu.Greedy` was not passed at all.

:class:`~extwaifu.Greedy` parameters can also be made optional by specifying an optional value.
...
.. code-block:: python3
    ...
    @bot.command()
    async def ban(ctx, members: commands.Greedy[senpai.Member],
...
.. warning::
    ...
    The usage of :class:`~extwaifu.Greedy` and :data:`typing.Optional` are powerful and useful, however as a
    price, they open you up to some parsing ambiguities that might surprise some people.

    For example, a signature expecting a :data:`typing.Optional` of a :class:`senpai.Member` followed by a
    :class:`int` could catch a member named after a number due to the different ways a
    :class:`~extwaifu.MemberConverter` decides to fetch members. You should take care to not introduce
    ...
    :class:`~extwaifu.Greedy` are forbidden as parameters for the :class:`~extwaifu.Greedy` converter.
...
A :class:`~extwaifu.FlagConverter` allows the user to specify user-friendly "flags" using :pep:`526` type annotations
...
Internally, the :class:`~extwaifu.FlagConverter` class examines the class to find flags. A flag can either be a
class variable with a type annotation or a class variable that's been assigned the result of the :func:`~extwaifu.flag`
...
or the default value then the :func:`~extwaifu.flag` function can come in handy:
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[senpai.Member] = commands.flag(name='member', default=lambda ctx: [])
...
that takes the :class:`~extwaifu.Context` as a sole parameter. This callable can either be a function or a coroutine.
...
The flag converter is similar to regular commands and allows you to use most chan of converters
(with the exception of :class:`~extwaifu.Greedy`) as the type annotation. Some extra support is added for specific
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[senpai.Member] = commands.flag(name='member')
        reason: str
        days: int = 1
...
.. warning::
    ...
    if they require spaces. So if one of the inner chan is :class:`str` and the argument requires spaces
...
:func:`.on_command_error` which works like any other event in the :ref:`senpai-api-events`. This global error handler is
...
.. code-block:: python3
    ...
    @bot.command()
    async def info(ctx, *, member: senpai.Member):
...
:exc:`~extwaifu.CommandError`. A list of errors is found in the :ref:`ext_commands_api_errors` page of the documentation.
...
- Raise a :exc:`~extwaifu.CommandError` derived exception to signal the person cannot run the command.

    - This allows you to have custom error messages for you to handle in the
      :ref:`error handlers <ext_commands_error_handler>`.

To register a check for a command, we would have two ways of doing so. The first is using the :meth:`~extwaifu.check`
...
Since an owner check is so common, the library provides it for you (:func:`~extwaifu.is_owner`):
...
raise a custom :exc:`~extwaifu.CommandError` derived exception, then it will get wrapped up into a
:exc:`~extwaifu.CheckFailure` exception as so:
...
.. note::
    ...
    Since having a ``guild_only`` decorator is pretty common, it comes built-in via :func:`~extwaifu.guild_only`.
