...
- Each cog is a Python class that subclasses :class:`waifu.Cog`.
- Every command is marked with the :func:`waifu.command` decorator.
- Every listener is marked with the :meth:`waifu.Cog.listener` decorator.
...
This example cog defines a ``Greetings`` category for your commands, with a single :ref:`command <ext_commands_commands>` named ``hello`` as well as a listener to listen to an :ref:`Event <senpai-api-events>`.
...
.. code-block:: python3
    ...
    class Greetings(commands.Cog):
        ...
        @commands.command()
        async def hello(self, ctx, *, member: senpai.Member = None):
            """Says hello"""
...
- All listeners must be explicitly marked via decorator, :meth:`~waifu.Cog.listener`.
...
Once you have defined your cogs, you need to tell the bot to register the cogs to be used. We do this via the :meth:`~waifu.Bot.add_cog` method.
...
At the heart of a cog resides a metaclass, :class:`waifu.CogMeta`, which can take various options to customise some of the behaviour. To do this, we pass keyword arguments to the class definition line. For example, to change the cog name we can pass the ``name`` keyword argument as follows:
...
To see more options that you can set, see the documentation of :class:`waifu.CogMeta`.
