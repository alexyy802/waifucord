...
.. code-block:: python3
    ...
    @bunny.event
    async def on_ready():
        print(f'We have logged in as {bunny.user}')
    ...
    bunny.run('your token here')

Let's name this file ``example_bot.py``. Make sure not to name it ``senpai.py`` as that'll conflict
with the library.
...
2. Next, we create an instance of a :class:`Bunny`. This bunny is our connection to Discord.
3. We then use the :meth:`Bunny.event` decorator to register an event. This library has many events.
...
4. Since the :func:`on_message` event triggers for *every* message received, we have to make
   ...
   is the same as the :attr:`Bunny.user`.
...
6. Finally, we run the bot with our login token. If you need help getting your token or creating a bot,
   look in the :ref:`senpai-intro` section.
