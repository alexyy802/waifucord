...
*senpai.py* logs errors and debug information via the :mod:`logging` python
...
senpai as well as other libraries that use the ``logging`` module
...
example to write the logs to a file called ``senpai.log`` instead of
outputting them to the console the following snippet can be used::
    ...
    handler = logging.FileHandler(filename='senpai.log', encoding='utf-8', mode='w')
